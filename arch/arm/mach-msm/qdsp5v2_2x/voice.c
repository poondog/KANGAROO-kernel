...
static int voice_cmd_change(void)
{
	...
	MM_AUD_INFO("[voice] send CMD_DEVICE_CHANGE\n");
			...
			void *private_data)
{
	...
	case AUDDEV_EVT_START_VOICE:
		MM_AUD_INFO("[dev ctrl] AUDDEV_EVT_START_VOICE\n");
		if ((v->dev_state == DEV_INIT) ||
			...
			if ((v->dev_rx.enabled == VOICE_DEV_ENABLED)
				...
				MM_AUD_INFO("dev_state -> DEV_READY\n");
				wake_up(&v->dev_wait);
	...
	case AUDDEV_EVT_DEV_CHG_VOICE:
		...
		if (v->dev_state == DEV_READY) {
			...
			if (v->voc_state == VOICE_ACQUIRE) {
				...
				if (rc == 0) {
					MM_AUD_INFO("wait timeout, voc_state = %d\n", v->voc_state);
					return;
				}
				MM_AUD_INFO("wait done, voc_state = %d\n", v->voc_state);
			} else {
				MM_AUD_INFO("Get AUDDEV_EVT_DEV_CHG_VOICE "
	...
	case AUDDEV_EVT_DEV_RDY:
		/* update the dev info */
		MM_AUD_INFO("[dev ctrl] AUDDEV_EVT_DEV_RDY\n");
	...
	case AUDDEV_EVT_DEVICE_VOL_MUTE_CHG:
		MM_AUD_INFO("[dev ctrl] AUDDEV_EVT_DEVICE_VOL_MUTE_CHG\n");
	...
	case AUDDEV_EVT_REL_PENDING:
		MM_AUD_INFO("[dev ctrl] AUDDEV_EVT_REL_PENDING, dev_state %d\n",
			v->dev_state);
		...
		if (v->dev_state == DEV_READY) {
			if (atomic_read(&v->rel_start_flag)) {
				...
				if (evt_payload->voc_devinfo.dev_type == DIR_RX)
					...
					v->dev_tx.enabled = VOICE_DEV_DISABLED;
				v->dev_state = DEV_REL_DONE;
				MM_AUD_INFO("dev_state -> DEV_REL_DONE\n");
				wake_up(&v->dev_wait);
	...
	case AUDDEV_EVT_END_VOICE:
		MM_AUD_INFO("[dev ctrl] AUDDEV_EVT_END_VOICE\n");
		/* recover the tx mute and rx volume to the default values */
		v->dev_tx.mute = v->default_mute_val;
		v->dev_rx.volume = v->default_vol_val;
		...
		if ((v->dev_state == DEV_READY) ||
			...
			if (atomic_read(&v->rel_start_flag)) {
				...
				MM_AUD_INFO("send voice_cmd_change at voc_state %d\n",
					v->voc_state);
				/* send mute and default volume value to MCAD */
				voice_cmd_device_info(v);
				/* send device change to modem */
	...
	case AUDDEV_EVT_FREQ_CHG:
		MM_AUD_INFO("[dev ctrl] AUDDEV_EVT_FREQ_CHG\n");
				...
				void *evt_buf, u32 len)
{
	...
	case EVENT_ACQUIRE_START:
		MM_AUD_INFO("[radio] EVENT_ACQUIRE_START\n");
	...
	case EVENT_RELEASE_START:
		MM_AUD_INFO("[radio] EVENT_RELEASE_START\n");
	...
	case EVENT_CHANGE_START:
		MM_AUD_INFO("[radio] EVENT_CHANGE_START\n");
		mutex_lock(&voice.lock);
	...
	case EVENT_NETWORK_RECONFIG:
		...
		MM_AUD_INFO("[radio] EVENT_NETWORK_CONFIG\n");
		mutex_lock(&voice.lock);
...
static int voice_cmd_init(struct voice_data *v)
{
	...
	MM_AUD_INFO("[voice] send CMD_ACQUIRE_INIT\n");
...
static int voice_cmd_acquire_done(struct voice_data *v)
{
	...
	MM_AUD_INFO("[voice] send CMD_ACQUIRE_DONE\n");
...
static int voice_cmd_release_done(struct voice_data *v)
{
	...
	MM_AUD_INFO("[voice] send CMD_RELEASE_DONE\n");
...
static int voice_cmd_device_info(struct voice_data *v)
{
	...
	MM_AUD_INFO("[voice] send CMD_DEVICE_INFO "
	...
	MM_AUD_INFO("rx dev_id = %d, tx_dev_id = %d,"
...
void voice_change_sample_rate(struct voice_data *v)
{
	...
	MM_AUD_INFO("%s: network %d, freq %d\n", __func__, v->network, freq);
...
static int voice_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		MM_AUD_INFO("handle voice event %d, "
		...
		if (v->dev_event != DEV_CHANGE_READY) {
			...
			case VOICE_ACQUIRE_START:
				...
				if ((v->voc_state == VOICE_INIT) ||
					...
					if (v->dev_state == DEV_READY) {
						...
						MM_AUD_INFO("voc_state -> VOICE_ACQUIRE\n");
					} else {
						MM_AUD_INFO("start waiting for "
							"dev_state -> DEV_READY\n");
						rc = wait_event_interruptible(
						...
						if (atomic_read(&v->rel_start_flag) == 1) {
							v->voc_state = VOICE_RELEASE;
							MM_AUD_INFO("voc_state -> VOICE_RELEASE\n");
							atomic_dec(&v->rel_start_flag);
					...
					MM_AUD_ERR("Get VOICE_ACQUIRE_START "
			...
			case VOICE_RELEASE_START:
				if ((v->dev_state == DEV_REL_DONE) ||
					...
					if (rc == 0) {
					MM_AUD_INFO("wait timeout, dev_state = %d\n", v->dev_state);
					}
			...
			case VOICE_CHANGE_START:
				if (v->voc_state == VOICE_ACQUIRE) {
					...
					MM_AUD_INFO("voc_state -> VOICE_CHANGE\n");
				} else
			...
			case VOICE_NETWORK_RECONFIG:
				if ((v->voc_state == VOICE_ACQUIRE)
					...
					MM_AUD_ERR("Get VOICE_NETWORK_RECONFIG "
					       "at wrong voc_state %d\n", v->voc_state);
				break;
			default:
				break;
		...
		case DEV_CHANGE_READY:
			MM_AUD_INFO("Get DEV_CHANGE_READY at voc_state %d\n",
				v->voc_state);
			if (v->voc_state == VOICE_CHANGE) {
				msm_snddev_enable_sidetone(v->dev_rx.dev_id, 1);
				...
				MM_AUD_INFO("Get DEV_CHANGE_READY "
...
static int __init voice_init(void)
{
	...
	v->dev_state = DEV_INIT;
	MM_AUD_INFO("dev_state -> DEV_INIT\n");
	v->voc_state = VOICE_INIT;
	MM_AUD_INFO("voc_state -> VOICE_INIT\n");
